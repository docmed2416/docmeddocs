...
function labelFor(view: View): string {
  ...
  return view.key.split("/").pop() ?? view.key;
}
...
export default function App() {
  ...
  // Fetch doc for current tab if not cached
  useEffect(() => {
    ...
    fetch(`/docs/${key}.json`)
      ...
      .then((data: DocFile | null) => {
        setDocCache((c) => ({ ...c, [key]: data }));
      })
      .catch(() => {
        setDocCache((c) => ({ ...c, [key]: null }));
      })
      .finally(() => {
    ...
    <div className="layout">
      ...
      <div className="main-column">
        ...
        <main className="main">
          ...
          {view.type === "doc" && (
            <DocView doc={currentDoc} loading={isLoadingDoc} />
          )}
        </main>
      </div>
    </div>
